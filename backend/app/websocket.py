...
def init_socketio(app):
    ...
    socketio.init_app(app,
                     ...
                     ping_timeout=60,  # Increased from default 60s
...
def on_disconnect():
    """Handle client disconnection safely without causing response errors."""
    try:
        ...
        for user_id, sid in list(connected_clients.items()):
            if sid == request.sid:
                ...
                logger.debug(f"User {user_id} removed from connected_clients")
                break

        # Remove from connected admins if applicable
        if request.sid in connected_admins:
            connected_admins.discard(request.sid)
            logger.debug("Admin removed from connected_admins")
    except (RuntimeError, AssertionError) as e:
        # RuntimeError or AssertionError expected when client disconnects mid-cleanup
        # "write() before start_response" is a Werkzeug WSGI timing issue in dev mode
        logger.debug(f"Expected disconnect cleanup error for {getattr(request, 'sid', 'unknown')}: {type(e).__name__}")
    except Exception as e:
        logger.error(f"Unexpected error during disconnect cleanup: {str(e)}", exc_info=False)
...
def broadcast_to_user(user_id, event, data):
    ...
    """
    try:
        if not hasattr(current_app, 'socketio') or current_app.socketio is None:
...
def broadcast_to_all(event, data):
    ...
    try:
        ...
        current_app.socketio.emit(event, data, namespace='/')
        logger.info(f"[v0] Broadcast to all clients: {event}")
    except Exception as e:
...
def check_namespace(request):
    ...
    If not, add a default namespace attribute.
