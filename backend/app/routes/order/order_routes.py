    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # If we get an enum error during pagination, it's likely due to invalid data in the database
                error_msg = str(enum_error)
                if 'is not among the defined enum values' in error_msg or 'enum' in error_msg.lower():
                    logger.error(f"[v0] Enum validation error during pagination: {error_msg}")
                    logger.error("[v0] This indicates invalid enum values in the database.")
                    logger.error("[v0] Run: python backend/scripts/fix-payment-status-enum-data.py to fix the data.")
                    
                    # Try to fix on-the-fly: fetch orders with lazy loading to avoid enum validation
                    logger.info("[v0] Attempting to fix enum values on-the-fly...")
                    try:
                        orders = query.all()
                        ...
                        for order in orders:
                            if isinstance(order.payment_status, str):
                                ...
                                status_str = order.payment_status.strip().lower()
                                if status_str in status_map:
                                    order.payment_status = status_map[status_str]
                        
                        db.session.commit()
                        logger.info("[v0] Fixed enum values successfully. Retrying query...")
                        ...
                        logger.error(f"[v0] Could not fix enum values on-the-fly: {str(fix_error)}")
                        return jsonify({
                            'success': False,
                            'error': 'Database contains invalid enum values. Please run the fix script.',
                            'details': 'Run: python backend/scripts/fix-payment-status-enum-data.py'
                        }), 500
                else:
                    raise enum_error
...
def get_user_order(order_id):
    ...
    try:
        user = get_current_user()
        logger.info(f"[v0] get_user_order called for order_id: {order_id}")
        logger.info(f"[v0] Current user: {user.id if user else 'None'}, Email: {user.email if user else 'None'}")
        
        if not user:
            logger.error("[v0] User not found in get_user_order")
            return jsonify({
                'success': False,
                'error': 'User not found'
            }), 404
        
        logger.info(f"[v0] Querying order with id={order_id}, user_id={user.id}")
        order = Order.query.filter_by(id=order_id, user_id=user.id).first()
        
        if not order:
            # Log diagnostic information about why order wasn't found
            logger.warning(f"[v0] Order {order_id} not found for user {user.id}")
            
            # Check if order exists at all (for debugging)
            any_order = Order.query.filter_by(id=order_id).first()
            if any_order:
                logger.warning(f"[v0] Order {order_id} exists but belongs to user {any_order.user_id}, not user {user.id}")
            else:
                logger.warning(f"[v0] Order {order_id} does not exist in database")
        ...
        logger.info(f"[v0] Order found: order_number={order.order_number}, status={order.status}, payment_status={order.payment_status}")
        ...
        logger.error(f"[v0] Error getting order {order_id}: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            if payment_method == 'cash_on_delivery':
                ...
                payment_status = PaymentStatus.PENDING  # Payment will be collected on delivery
            else:
                order_status = OrderStatus.PENDING  # Other payment methods start as pending
        ...
        if payment_method == 'cash_on_delivery':
            try:
                ...
                logger.info(f"[v0] 💳 COD Order created! Sending order confirmation email to {user.email}")
                logger.info(f"[v0] Order ID: {order.id}, Customer: {customer_name}")
                ...
                if email_sent:
                    logger.info(f"[v0] ✅ Order confirmation email sent successfully for COD order")
                else:
                    logger.error(f"[v0] ❌ Failed to send order confirmation email for COD order")
            except Exception as email_error:
                logger.error(f"[v0] Exception sending confirmation email for COD order: {str(email_error)}", exc_info=True)
...
def cancel_order(order_id):
    ...
    try:
        ...
        if isinstance(order_status, str):
            # Status is stored as string, convert to lowercase for comparison
            status_value = order_status.lower()
        else:
            # Status is enum, get its value
            status_value = order_status.value if hasattr(order_status, 'value') else str(order_status).lower()
        
        # Check if order can be cancelled
        cancellable_statuses = ['pending', 'confirmed', 'processing']
        if status_value not in cancellable_statuses:
            return jsonify({
                'success': False,
                'error': f'Cannot cancel order with status: {status_value}'
            }), 400
        
        # Update order status to cancelled
        order.status = 'cancelled'  # Store as string to match database format
...
def track_order(order_id):
    ...
    try:
        ...
        tracking_info = {
            'order': order.to_dict(),
            'timeline': timeline,
            'current_status': order.status.value if hasattr(order.status, 'value') else str(order.status),  # Convert enum to string
            'tracking_number': order.tracking_number
        }
