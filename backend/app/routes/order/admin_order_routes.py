...
try:
    ...
    class Message:
        """Placeholder for Flask-Mail Message class if not available"""
...
def _status_value(s):
   """Return a safe string value for a status enum or plain string.

   Works with: enum members with .value, plain strings, or None.
   """
   try:
       if isinstance(s, str):
           return s
...
def format_order_response(order, include_items=False, include_user=True, include_history=False, include_attachments=False):
   ...
   try:
       ...
       order_dict = {
           ...
           'payment_status': order.payment_status if isinstance(order.payment_status, str) else (order.payment_status.value if order.payment_status else None),
           ...
           'allowed_transitions': [
               ...
               for status in ALLOWED_STATUS_TRANSITIONS.get(order.status, [])
           ],
...
def update_order_status(order_id):
   ...
   try:
       ...
       if not validate_status_transition(old_status, new_status):
           return jsonify({
               "error": f"Invalid status transition from {old_status} to {new_status.value}",
       ...
       try:
           ...
           logger.info(f"WebSocket notifications sent for order {order_id} status update: {_status_value(old_status)} -> {_status_value(new_status)}")
       ...
       log_admin_activity(
           current_user_id,
           'UPDATE_ORDER_STATUS',
           f'Updated order_id:{order_id} status from {old_status if isinstance(old_status, str) else old_status.value} to {new_status if isinstance(new_status, str) else new_status.value}. Notes: {note_text}'
       )
       ...
       response_data = {
           ...
           "order": {
               ...
               'updated_at': order.updated_at.isoformat(),
               'allowed_next_transitions': [status.value if hasattr(status, 'value') else status for status in ALLOWED_STATUS_TRANSITIONS.get(order.status, [])]
           }
       }
