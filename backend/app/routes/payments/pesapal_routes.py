...
def initiate_card_payment():
    ...
    try:
        ...
        # This prevents IntegrityError due to unique constraints on merchant_reference or other fields
        existing_transactions = PesapalTransaction.query.filter(
            and_(
                PesapalTransaction.order_id == order_id,
                PesapalTransaction.user_id == current_user_id,
                PesapalTransaction.status.in_(['pending', 'initiated', 'failed'])
            )
        ).all()
        ...
        for existing_transaction in existing_transactions:
            ...
            if expires_at and datetime.now(timezone.utc) > expires_at:
                ...
                logger.info(f"Cancelled expired transaction {existing_transaction.id} for order {order_id}")
            else:
                existing_transaction.status = 'cancelled'
                existing_transaction.error_message = 'Superseded by new payment attempt'
                logger.info(f"Cancelled existing {existing_transaction.status} transaction {existing_transaction.id} for order {order_id}")
        
        if existing_transactions:
            db.session.commit()
            logger.info(f"Cancelled {len(existing_transactions)} existing transaction(s) for order {order_id}")

        merchant_reference = None
        max_retries = 10
        for attempt in range(max_retries):
            # Include timestamp in reference to avoid conflicts with cancelled transactions
            timestamp_suffix = datetime.now(timezone.utc).strftime('%H%M%S%f')[:10]
            merchant_reference = f"{generate_unique_merchant_reference(order_id, current_user_id)}_{timestamp_suffix}"
        ...
        try:
            ...
            if 'localhost' in callback_url or '127.0.0.1' in callback_url:
                ...
                logger.info(f"[v0] ✅ Development mode detected - localhost callback URL allowed: {callback_url}")
            elif PESAPAL_CONFIG['environment'] == 'production':
                # Production mode - require real domain
            ...
            try:
                ...
                logger.info(f"Function: create_card_payment_request")
                logger.info(f"Module: {create_card_payment_request.__module__}")
            ...
            try:
                # Cancel any transactions that might have been created in a race condition
                conflicting_txns = PesapalTransaction.query.filter(
                    and_(
                        PesapalTransaction.order_id == order_id,
                        PesapalTransaction.user_id == current_user_id,
                        PesapalTransaction.status.in_(['pending', 'initiated', 'failed'])
                ...
                for txn in conflicting_txns:
                    txn.status = 'cancelled'
                    txn.error_message = 'Cancelled due to conflict resolution'
                
                if conflicting_txns:
                    db.session.commit()
                    logger.info(f"Cleaned up {len(conflicting_txns)} conflicting transactions - please retry")
                
                return create_error_response(
                    'Payment system busy. Please try again.',
                    503, 'RETRY_PAYMENT'
                )
            except Exception as cleanup_error:
                logger.error(f"Failed to clean up conflicting transactions: {str(cleanup_error)}")
                return create_error_response(
...
def check_card_payment_status(transaction_id):
    ...
    try:
        ...
        try:
            ...
            pass  # No authentication provided, continue without user_id
        ...
        try:
            ...
            pass  # Not a UUID, try as OrderTrackingId
        ...
        logger.info(f"[v0] Transaction found - Status: {transaction.status}, Order: {transaction.order_id}")
        ...
        if transaction.status in ['completed', 'failed', 'cancelled', 'expired']:
            logger.info(f"[v0] Transaction in final state: {transaction.status}")
            transaction_data = {
                'id': transaction.id,
            ...
            return create_success_response({
                ...
                'payment_status': transaction.status.upper(),  # Added for compatibility
                'transaction_data': transaction_data
            }, get_payment_status_message(transaction.status))

        # Query Pesapal for current status
        logger.info(f"[v0] Querying Pesapal for current status of tracking ID: {transaction.pesapal_tracking_id}")
        if hasattr(transaction, 'pesapal_tracking_id') and transaction.pesapal_tracking_id:
            try:
                ...
                logger.info(f"[v0] Pesapal API response received: {json.dumps(status_response, indent=2)}")
                
                if status_response:
                    # get_transaction_status() returns: {'status': 'success/error', 'data': {...}}
                    # The actual Pesapal fields are in 'data' or 'data.response'
                    response_data = {}
                    if status_response.get('status') == 'success':
                        # Success response - data is at top level of status_response
                        response_data = status_response
                    else:
                        # Check if data is nested in 'data' key
                        response_data = status_response.get('data', {})
                        # Check if further nested in 'response' key
                        if 'response' in response_data:
                            response_data = response_data.get('response', {})
                    
                    # Extract payment status from the actual Pesapal response
                    payment_status = (response_data.get('payment_status_description') or '').upper().strip()
                    if not payment_status:
                        payment_status = (response_data.get('payment_status') or '').upper().strip()
                    
                    status_code = response_data.get('status_code')
                    
                    logger.info(f"[v0] Extracted payment_status: '{payment_status}', status_code: {status_code}")
                    
                    # Check if response indicates success
                    response_success = (
                        status_response.get('status') == 'success' or 
                        status_response.get('status') == '200' or 
                        response_data.get('message') == 'Request processed successfully' or
                        response_data.get('status') == '200' or
                        status_code == 1 or  # status_code 1 means completed
                        payment_status in ['COMPLETED', 'COMPLETE', 'SUCCESS']
                    )
                    
                    logger.info(f"[v0] Response success check: {response_success}, payment_status: '{payment_status}', status_code: {status_code}")
                    
                    if response_success and (payment_status in ['COMPLETED', 'COMPLETE', 'SUCCESS'] or status_code == 1):
                        # Card payment successful
                        logger.info(f"[v0] ✅ Payment completed! Updating transaction status from '{transaction.status}' to 'completed'")
                        transaction.status = 'completed'
                        transaction.payment_method = response_data.get('payment_method', 'CARD')
                        transaction.pesapal_receipt_number = response_data.get('confirmation_code')
                        ...
                        if transaction.order_id:
                            try:
                                ...
                                logger.error(f"[v0] Error updating order status: {str(order_error)}")
                                # Continue with email sending even if order update fails

                        logger.info(f"[v0] ✅ Card payment completed for transaction {transaction.id}")
                    ...
                    elif response_success and (payment_status in ['FAILED', 'FAIL', 'ERROR'] or status_code == 0):
                        logger.info(f"[v0] ❌ Payment failed! Updating transaction status from '{transaction.status}' to 'failed'")
                        transaction.status = 'failed'
                        transaction.error_message = response_data.get('error_message', 'Card payment failed')
                        logger.info(f"[v0] ❌ Card payment failed for transaction {transaction.id}")

                    elif response_success and (payment_status in ['CANCELLED', 'CANCELED', 'CANCEL']):
                        logger.info(f"[v0] ❌ Payment cancelled! Updating transaction status from '{transaction.status}' to 'cancelled'")
                        transaction.status = 'cancelled'
                        logger.info(f"[v0] ❌ Card payment cancelled for transaction {transaction.id}")
                    
                    elif response_success and (payment_status in ['PENDING', 'PROCESSING', 'INVALID'] or status_code == 2):
                        # Keep transaction as pending - INVALID often means transaction is too new
                        if transaction.status != 'pending':
                            logger.info(f"[v0] ⏳ Payment still processing! Updating transaction status from '{transaction.status}' to 'pending'")
                            transaction.status = 'pending'
                        logger.info(f"[v0] ⏳ Card payment still pending for transaction {transaction.id} (status: {payment_status}, code: {status_code})")

                    # Store status response
                    transaction.status_response = json.dumps(response_data) # Store the extracted response_data
                    transaction.last_status_check = datetime.now(timezone.utc)
                    db.session.commit()
                    logger.info(f"[v0] ✅ Transaction {transaction.id} committed to database with status: {transaction.status}")

            except Exception as e:
                logger.error(f"[v0] Error querying Pesapal status for transaction {transaction.id}: {str(e)}")
                import traceback
        ...
        logger.info(f"[v0] Returning transaction status: {transaction.status}")
        return create_success_response({
            'transaction_status': transaction.status,
            'transaction_data': transaction_data
        }, get_payment_status_message(transaction.status))
        ...
        logger.error(f"[v0] Error checking card payment status: {str(e)}")
...
def get_card_payment_stats():
    ...
    try:
        ...
        card_types_query = card_types_query.group_by(PesapalTransaction.card_type).all()
        ...
        if group_by in ['day', 'week', 'month']:
            ...
            trend_start_date = from_date_obj if from_date_obj else datetime.now(timezone.utc) - timedelta(days=6)
            trend_end_date = to_date_obj if to_date_obj else datetime.now(timezone.utc)
            
            # Ensure the trend range is reasonable if no dates are provided
            if not from_date and not to_date:
                trend_start_date = datetime.now(timezone.utc) - timedelta(days=6)
                trend_end_date = datetime.now(timezone.utc)
            elif from_date and not to_date:
                trend_end_date = datetime.now(timezone.utc)
            elif not from_date and to_date:
                trend_start_date = trend_end_date - timedelta(days=6) # Default to 7 days if only end date is given
            
            # Adjust to start of day for accurate filtering
            trend_start_date = trend_start_date.replace(hour=0, minute=0, second=0, microsecond=0)
            trend_end_date = trend_end_date.replace(hour=23, minute=59, second=59, microsecond=999999) # End of the day
            ...
            while current_trend_date <= trend_end_date:
                ...
                day_transactions = day_query.count()
