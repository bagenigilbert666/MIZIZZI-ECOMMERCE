...
def create_category():
    ...
    try:
        ...
        try:
            broadcast_to_all('category_created', {'category': get_category_with_stats(new_category)})
        except Exception:
            logger.exception('Failed to broadcast category_created event')
...
def update_category(category_id):
    ...
    try:
        ...
        try:
            broadcast_to_all('category_updated', {'category': get_category_with_stats(category)})
        except Exception:
            logger.exception('Failed to broadcast category_updated event')
...
def delete_category(category_id):
    ...
    try:
        ...
        # Notify clients that a category was deleted
        try:
            broadcast_to_all('category_deleted', {'id': category_id, 'name': category_name})
        except Exception:
            logger.exception('Failed to broadcast category_deleted event')
    ...
    try:
        ...
        for item in category_orders:
            ...
            if category:
                ...
                category.updated_at = datetime.utcnow()
                updated_count += 1
                updated_list.append({'id': category.id, 'sort_order': category.sort_order})

        db.session.commit()

        # Broadcast reorder event with the new order
        try:
            broadcast_to_all('categories_reordered', {'updated': updated_list})
        except Exception:
            logger.exception('Failed to broadcast categories_reordered event')
...
# Compatibility upload endpoint (alias) used by frontend: /api/admin/categories/upload-image
@admin_category_routes.route('/upload-image', methods=['POST', 'OPTIONS'])
@cross_origin()
@jwt_required()
@admin_required
def upload_category_image_alias():
    """Alias upload endpoint to support legacy frontend path: saves to uploads/categories and returns URL."""
    if request.method == 'OPTIONS':
        return handle_options_request()
    ...
    try:
        ...
        configured = current_app.config.get('UPLOAD_FOLDER', 'uploads') or 'uploads'
        if configured.endswith('categories'):
            upload_folder = configured
        else:
            upload_folder = os.path.join(configured, 'categories')
        ...
        try:
            ...
            logger.exception(f"Failed to save uploaded file to {filepath}: {str(e)}")
            return jsonify({'error': 'Failed to save uploaded file', 'detail': str(e)}), 500

        file_url = f"/api/uploads/categories/{filename}"
        return jsonify({
        ...
        logger.exception(f"Error uploading image (alias route): {str(e)}")
