...
def get_admin_products():
    ...
    try:
        ...
        if is_active is None:
            # By default, only show active products (not soft-deleted)
            query = query.filter(Product.is_active == True)
        else:
            # If explicitly specified, filter by that value
...
def create_product():
    ...
    try:
        ...
        product.is_daily_find = data.get('is_daily_find', False)
        product.is_top_pick = data.get('is_top_pick', False)
        product.is_trending = data.get('is_trending', False)
        product.is_new_arrival = data.get('is_new_arrival', False)
...
def update_product(product_id):
    ...
    try:
        ...
        for field in ['is_featured', 'is_new', 'is_sale', 'is_flash_sale',
                     'is_luxury_deal', 'is_daily_find', 'is_top_pick', 'is_trending', 'is_new_arrival', 'is_active', 'is_digital', 'is_taxable',
...
@admin_products_routes.route('/<int:product_id>/images', methods=['GET'])
@admin_required
def get_product_images(product_id):
    ...
    try:
        ...
        current_app.logger.info(f"Retrieved {len(images)} images for product {product_id}")
        ...
        current_app.logger.error(f"Error retrieving images for product {product_id}: {str(e)}")
        return jsonify({'error': 'Internal server error'}), 500

@admin_products_routes.route('/<int:product_id>/images', methods=['POST'])
@admin_required
def add_product_image(product_id):
    """Add an image to a product and update Product table cache."""
    try:
        product = db.session.get(Product, product_id)
        ...
        db.session.add(image)
        db.session.flush()  # Flush to get the image ID before updating Product
        
        # Get all current images including the new one
        ...
        # Update thumbnail_url to the primary image or first image
        if all_images:
            primary_image = next((img for img in all_images if img.is_primary), None)
            product.thumbnail_url = primary_image.url if primary_image else all_images[0].url
        ...
        try:
            ...
            current_app.logger.info(f"WebSocket notification sent for product {product_id} image update")
        except Exception as ws_error:
            # Don't fail the request if WebSocket fails
            current_app.logger.warning(f"WebSocket notification error: {str(ws_error)}")
...
def delete_product_image(image_id):
    """Delete a product image and update Product table cache."""
    try:
        image = db.session.get(ProductImage, image_id)
        ...
        # Delete the image record from product_images table
        db.session.delete(image)
        
        product = db.session.get(Product, product_id)
        current_urls = [] # Initialize to handle cases where product might not exist after delete
        if product:
            # Update image_urls array to remove the deleted URL
            current_urls = product.get_image_urls() or []
            if deleted_url in current_urls:
                current_urls.remove(deleted_url)
                product.set_image_urls(current_urls)
            
            # Update thumbnail_url if it was the deleted image
            if product.thumbnail_url == deleted_url:
                # Get remaining images for this product
                remaining_images = ProductImage.query.filter_by(product_id=product_id).filter(
                    ProductImage.id != image_id
                ).order_by(ProductImage.is_primary.desc(), ProductImage.sort_order.asc()).all()
                
                if remaining_images:
                    # Set thumbnail to the first remaining image (primary or first by sort order)
                    product.thumbnail_url = remaining_images[0].url
                else:
        ...
        try:
            ...
            current_app.logger.info(f"WebSocket notification sent for product {product_id} image deletion")
        except Exception as ws_error:
            # Don't fail the request if WebSocket fails
            current_app.logger.warning(f"WebSocket notification error: {str(ws_error)}")
...
def get_product_stats():
    ...
    try:
        stats = {
            ...
            'daily_find_products': Product.query.filter_by(is_daily_find=True, is_active=True).count(),
            'top_pick_products': Product.query.filter_by(is_top_pick=True, is_active=True).count(),
            'trending_products': Product.query.filter_by(is_trending=True, is_active=True).count(),
            'new_arrival_products': Product.query.filter_by(is_new_arrival=True, is_active=True).count(),
