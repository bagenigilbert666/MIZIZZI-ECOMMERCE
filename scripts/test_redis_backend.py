...
def load_env_file():
    """Load environment variables from a discovered .env file.

    This function searches a small set of likely locations (in order):
      - the script directory (backend/scripts/.env)
      - the script parent (backend/.env)
      - the script parent's parent (project root /.env)
      - the current working directory (where the user runs the command)

    It uses python-dotenv if available, otherwise falls back to a simple parser.
    """
    ...
    # If none found, default to backend/.env (keeps previous behavior but without doubling)
    if env_file is None:
    ...
    try:
        ...
        print(f"✓ Loaded environment from {env_file.resolve()} (using python-dotenv)\n")
        return True
    except ImportError:
        ...
        if env_file.exists():
            ...
            print(f"✓ Loaded environment from {env_file.resolve()} (manual parsing)\n")
            return True
    # Nothing loaded
    print(f"✗ .env not found at any of: {', '.join(str(p.resolve()) for p in candidates)}")
    return False
