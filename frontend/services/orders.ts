...
export const orderService = {
  ...
  async cancelOrder(orderId: string, reason?: string): Promise<boolean> {
    try {
      ...
      console.log("[v0] Sending cancel request for order:", apiId)

      // Send empty body or just status to avoid backend enum parsing issues
      const response = await api.post(`/api/orders/${apiId}/cancel`, {})

      console.log("[v0] Cancel order response:", response.data)
      ...
      const errorMessage =
        error.response?.data?.message || error.response?.data?.error || error.message || "Failed to cancel order"
