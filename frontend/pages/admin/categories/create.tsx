        ...
        try {
          if (adminService && typeof (adminService as any).getShopCategories === "function") {
            ...
            if (!response.ok) {
              ...
              if (response.status === 401 && adminService && typeof (adminService as any).getCategories === "function") {
                try {
...
// New: fetch categories with public-first, admin-fallback strategy
...
export async function invalidateCategories() {
  ...
  // Trigger SWR revalidation across all instances
  await globalMutate(cacheKey)
  console.log("[v0] Categories cache invalidated")
}
...
;(async function initCategorySocketListeners() {
  try {
    ...
    // Try common global attachment points first (no bundler resolution)
    const socketFromWindow = win.__MIZIZZI_SOCKET__ || win.socket || win.io || null
    const attachListeners = (socket: any) => {
      if (!socket || typeof socket.on !== "function") return
      const invalidate = () => {
        try {
          globalMutate(CATEGORIES_CACHE_KEY)
          adminDataCache.delete(CATEGORIES_CACHE_KEY)
        } catch (e) { /* ignore */ }
      }
      socket.on("category_created", invalidate)
      socket.on("category_updated", invalidate)
      socket.on("category_deleted", invalidate)
      console.log("Category socket listeners attached (window)")
    }

    if (socketFromWindow) {
      attachListeners(socketFromWindow)
      return
    }

    // Fallback: attempt a runtime dynamic import without using a static string so webpack won't resolve it at build time.
    // Module path can be overridden via NEXT_PUBLIC_SOCKET_MODULE if needed.
    const modulePath = (process.env.NEXT_PUBLIC_SOCKET_MODULE as string) || "@/lib/socket"
    try {
      // Use eval to avoid static analysis by the bundler
      // @ts-ignore
      const mod = await eval('import(modulePath)').catch(() => null)
      const socket = (mod && (mod.default || mod.socket || mod.io)) || null
      if (socket && typeof socket.on === "function") {
        attachListeners(socket)
        console.log("Category socket listeners attached (import)")
      }
    } catch (e) {
      // Import failed or socket not available — silently ignore (non-fatal)
    }
  } catch (e) {
