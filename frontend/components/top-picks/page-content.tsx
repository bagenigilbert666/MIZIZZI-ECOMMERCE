...
/* ─── Types ─── */
interface TopPicksProduct extends Product {
  category?: string
  rating?: number
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="jumia-img-wrap relative aspect-square">
            ...
            <div className="absolute top-1.5 left-1.5 flex flex-col gap-1 z-10">
              ...
              {((product as TopPicksProduct).rating ?? 0) >= 4.5 && (
                <span className="jumia-badge jumia-new-tag inline-flex items-center gap-0.5 bg-amber-500 text-white text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded">
        ...
        >
          {/* Category tag */}
          {(product as TopPicksProduct).category && (
...
/* ─── Main Top Picks Page ─── */
export function TopPicksPageContent({
  products: initialProducts,
}: TopPicksPageContentProps) {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeFilter === "trending") {
      result = result.filter(
        (p) => ((p as TopPicksProduct).rating || 3) >= 4
      )
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === "rating") {
        return ((b as TopPicksProduct).rating || 3) - ((a as TopPicksProduct).rating || 3)
      }
      return 0
    })
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container py-8 px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 mb-8">
            ...
            <Star className="h-6 w-6 text-[#8B1538]" />
          </div>
        ...
        <div className="mb-5 sm:mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500">
              Our carefully curated selection of top products
            </p>
          </div>
        ...
        >
          {filteredProducts.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-gray-500 text-sm mb-2">No picks found</p>
              <button
                onClick={() => setSearchQuery("")}
