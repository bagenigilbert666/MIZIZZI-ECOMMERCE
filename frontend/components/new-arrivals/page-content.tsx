...
/* ─── Types ─── */
interface NewArrivalsProduct extends Product {
  category?: string
  rating?: number
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Category tag */}
          {(product as NewArrivalsProduct).category && (
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeFilter === "trending") {
      result = result.filter(
        (p) =>
          ((p as NewArrivalsProduct).rating || 3) >= 4 ||
          (p.sale_price != null && Number(p.sale_price) < Number(p.price))
      )
    } else if (activeFilter === "featured") {
      result = result.filter(
        (p) => p.sale_price != null && calculateDiscount(p.price, p.sale_price) >= 15
      )
    }
    ...
    result.sort((a, b) => {
      ...
      if (sortBy === "rating") {
        return ((b as NewArrivalsProduct).rating || 3) - ((a as NewArrivalsProduct).rating || 3)
      }
      return 0
    })
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container py-8 px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 mb-8">
            ...
            <Sparkles className="h-6 w-6 text-[#8B1538]" />
          </div>
        ...
        <div className="mb-5 sm:mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500">
              Discover the latest and newest products
            </p>
          </div>
