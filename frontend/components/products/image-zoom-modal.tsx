...
function toHighRes(url?: string): string {
  ...
  if (url.startsWith("blob:")) {
    console.warn(`[v0] Blob URL detected in image zoom modal, using placeholder`)
...
export function ImageZoomModal({ product, isOpen, onClose, selectedImageIndex }: ImageZoomModalProps) {
  ...
  const validImages = rawImages.filter((url: string) => !url.startsWith("blob:"))
  const images = validImages.length ? validImages : ["/generic-product-display.png"]
