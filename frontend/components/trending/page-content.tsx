...
import { TrendingBannerCarousel } from "@/components/trending/banner-carousel"
...
const JumiaProductCard = memo(function JumiaProductCard({
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const result = await addToCart(productIdNumber, 1)
      if (result?.success) {
        toast({
          title: "Added to Cart",
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="jumia-img-wrap relative aspect-square">
            ...
            <div className="absolute top-1.5 left-1.5 flex flex-col gap-1 z-10">
              ...
              {discount >= 15 && (
                <span className="jumia-badge jumia-new-tag inline-flex items-center gap-0.5 bg-red-500 text-white text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded">
        ...
        >
          {/* Category tag */}
          {(product as TrendingProduct).category && (
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeFilter === "featured") {
      result = result.filter(
        (p) => p.sale_price !== null && calculateDiscount(p.price, p.sale_price) >= 20
      )
    } else if (activeFilter === "toprated") {
      result = result.filter(
        (p) => ((p as TrendingProduct).rating || 3) >= 4
      )
    }
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container py-8 px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 mb-8">
            ...
            <Flame className="h-6 w-6 text-[#8B1538]" />
          </div>
        ...
        <div className="mb-5 sm:mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500">
              Hot products that everyone is looking for
            </p>
          </div>
        ...
        >
          {filteredProducts.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-gray-500 text-sm mb-2">No trending items found</p>
              <button
                onClick={() => setSearchQuery("")}
