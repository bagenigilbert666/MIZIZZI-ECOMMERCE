...
export function CashDeliveryPayment({
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const targetUrl = redirectUrl || `/order-confirmation/${orderIdToUse}`
      ...
      setTimeout(async () => {
        try {
          await clearCart()
          console.log("[v0] Cart cleared after COD payment confirmation")
        } catch (clearError) {
          console.error("[v0] Error clearing cart after COD payment:", clearError)
        }
      }, 500)
        ...
        {!success && (
          <>
            ...
            <Button
              ...
              className={
                !orderIdToUse
                  ? "bg-gradient-to-r from-rose-600 to-rose-700 hover:from-rose-700 hover:to-rose-800 text-white shadow-md"
