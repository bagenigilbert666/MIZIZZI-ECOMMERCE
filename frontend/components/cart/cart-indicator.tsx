...
export function CartIndicator() {
  ...
  const { validationResult } = useCartValidation()
  ...
  document.addEventListener("cart-updated", handleCartUpdate as unknown as EventListener)
    ...
    return () => {
  document.removeEventListener("cart-updated", handleCartUpdate as unknown as EventListener)
    ...
    <div className="relative">
      <Link href="/cart" className="relative">
        <Button
          variant="ghost"
          size="icon"
          className={`relative transition-all duration-300 flex items-center gap-1.5 px-2 sm:px-3 h-10 hover:bg-gray-100 rounded-md ${isAnimating ? "bg-cherry-50" : ""}`}
        ...
        >
          ...
          <AnimatePresence>
            {itemCount > 0 && (
              ...
              >
                <Badge
                  className={`h-5 min-w-[20px] px-1 flex items-center justify-center text-white text-xs font-bold rounded-full
                    ${isAnimating ? "bg-cherry-900 ring-2 ring-cherry-200" : "bg-cherry-900"}`}
                >
                  {itemCount}
                </Badge>
              </motion.div>
