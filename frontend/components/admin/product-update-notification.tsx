...
export function ProductUpdateNotification({ showToasts = true }: ProductUpdateNotificationProps) {
  ...
  window.addEventListener("product-updated", handleCustomEvent as unknown as EventListener)

    return () => {
      unsubscribe()
  window.removeEventListener("product-updated", handleCustomEvent as unknown as EventListener)
    }
  }, [showToasts])
