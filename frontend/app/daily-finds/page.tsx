...
const carouselItems = [
  ...
  },
  {
    ...
    description: "Discover unique products daily",
  },
]

export default function DailyFindsPage() {
  ...
  // Fetch daily find products
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true)
        setError(null)
        const result = await productService.getDailyFindProducts(50)
        setProducts(result)
        setHasMore(result.length >= 20)
      } catch (error) {
        console.error("Error fetching daily finds:", error)
        setError("Failed to load daily finds. Please try again.")
      } finally {
        setLoading(false)
      }
    }
  ...
  const loadMore = async () => {
    try {
      ...
      const moreProducts = await productService.getDailyFindProducts(50)
      ...
      <div className="container py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-neutral-500">
            <span className="font-medium text-neutral-900">{filteredProducts.length}</span> daily finds
          </p>
