...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Uploading image to Cloudinary via backend:", file.name)
    ...
    if (!backendResponse.ok) {
      ...
      console.error("[v0] Backend image upload error:", errorData)
      return NextResponse.json(
        {
    ...
    console.log("[v0] Image uploaded successfully:", uploadResult)
    ...
    console.error("[v0] Image upload API error:", error)
    return NextResponse.json(
      {
